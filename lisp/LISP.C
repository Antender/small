...
#define ERROR -1
enum inputTypes {
  INQUOTE=0,LPAREN,RPAREN,LETTER,DIGIT
};
  ...
  LIST *params, *args, *alist;	int prog;
{
  ...
  p->u.pname = (char *)getname(car(car(params)));
  rplact(p, VARI);
  if (prog)
...
{
  register LIST *p;   char *malloc();
  p = cons(NULL, NULL);
  strcpy(p->u.pname = malloc(strlen(name)+1), name);
  rplact(p, VARI);
  alist = cons(p, alist);
  return(p);
}
  ...
  register LIST *x;
{
  ...
  for (p = cdr(x);  p != NULL;  p = cdr(p))
    if (eval(car(p),NULL) == NULL)  return(NULL);
  return(TRU);
}
  ...
  LIST *x;
{
  register LIST *p;
  for (p = cdr(x);  p != NULL;  p = cdr(p))
    if(eval(car(p),NULL) != NULL)  return(TRU);
  return(NULL);
}
